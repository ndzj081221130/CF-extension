    ...
    class App < Sequel::Model #数据库?
      ...
      def react_to_saved_changes(changes)
        if changes.has_key?(:state)
          ...
          puts "react_to_saved_changes , changes = #{changes}"
          delta = changes[:instances][1] - changes[:instances][0]
          react_to_instances_change(delta)
        elsif changes.has_key?(:instances) && changes.has_key?(:update)
          puts "react_to_saved_changes , changes = #{changes} , update"
          delta = changes[:instances][1] - changes[:instances][0]
          react_to_instances_change(delta)
        end
      end
      ...
      def react_to_instances_change(delta)
        puts "react_to_instaces_change , delta = #{delta}"
        if started?
          stage_if_needed do
