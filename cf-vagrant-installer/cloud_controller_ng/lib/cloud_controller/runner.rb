...
module VCAP::CloudController
  class Runner
    ...
    def start_thin_server(app, config, message_bus)
      ...
      logger.debug2("start thin server address = #{@config[:bind_address]} , port =#{@config[:port]}")
      puts "start thin server address = #{@config[:bind_address]} , port =#{@config[:port]}"
