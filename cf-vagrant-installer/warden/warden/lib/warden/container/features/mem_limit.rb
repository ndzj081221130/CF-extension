  ...
  module Container
    ...
    module Features
      ...
      module MemLimit
        ...
        class OomNotifier
          ...
          def initialize(container)
            ...
            @child = DeferredChild.new(oom_notifier_path, container.cgroup_path(:memory))
              ...
              #    logger.debug("---zhang:container.cgroup_path(:memory) = #{container.cgroup_path(:memory)}")

               #   logger.debug("---zhang:oom_notifier_path = #{oom_notifier_path}")
            ...
            @child.callback do
              ...
              if @child.success?
                Fiber.new do
                  logger.debug("---zhang:container oomed ")
                  container.oomed
                end.resume
              end
            end
          ...
          def kill
            ...
            logger.debug("---zhang Killing oom-notifier process")
        ...
        def oomed
          logger.warn("---zhang! OOM happened for #{handle}")
        ...
        def start_oom_notifier_if_needed
          unless @oom_notifier # if @oom_notifier != null , skip the codes below
            @oom_notifier = OomNotifier.new(self)

            on(:after_stop) do
              if @oom_notifier
                logger.debug("---zhang in start oom notifier method")
                @oom_notifier.kill
                @oom_notifier = nil
              end
            end
        ...
        def limit_memory(limit_in_bytes)
         # logger.debug("---zhang: limit=#{limit_in_bytes}")
        ...
        def do_limit_memory(request, response)
         ...
         # logger.debug("---zhang request limit=#{request.limit_in_bytes}")
          limit_in_bytes = File.read(File.join(cgroup_path(:memory), "memory.limit_in_bytes"))
          #logger.debug("--zhang: #{cgroup_path(:memory)}  , #{limit_in_bytes}")
