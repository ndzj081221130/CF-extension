    ...
    class Base
      ...
      def hook(name, request, response, &blk)
        if respond_to?(name)
          ...
          m = method(name)
          #logger.debug(" ---zj method name :#{name}, method:#{m}" )
          if m.arity == 2
      ...
      def before_destroy
        ...
        begin
          logger.debug("--- zhang in before_destroy next is InfoRequest")
        ...
        unless self.state == State::Stopped
          ...
          begin
            logger.debug("--- zhang in before_destroy next is StopRequest")
      ...
      def do_run(request, response)
        ...
        #logger.debug2("---zhang: spawn : script = #{request.script}" )
        spawn_response = dispatch(spawn_request)
        #logger.debug2("---zhang: spawn response = #{spawn_response.job_id}")
