...
class LanguagePack::Ruby < LanguagePack::Base
  ...
  JVM_BASE_URL        = "http://114.212.85.89:80"#heroku-jvm-langpack-java.s3.amazonaws.com
  ...
  def compile
    ...
    # puts "install jvm"
    install_jvm
    # puts "before setup_language_apck_environment"
...
private
  ...
  def slug_vendor_base
    if @slug_vendor_base
      ...
      # @slug_vendor_base = ".rbenv/versions/1.9.3-p392/lib/ruby/gems/1.9.1"
      #
...
ERROR
    ...
    if build_ruby?
      ...
      # puts "build_ruby_path= #{build_ruby_path}"
  ...
  def install_language_pack_gems
    # puts "in install language pack gems"
  ...
  def build_bundler
    log("bundle") do
      ...
      if $?.success?
        ...
        puts "in build bundler Cleaning up the bundler cache."
        pipe "bundle clean"
        cache_store ".bundle"
...
<% end %>
  ...
  def add_bundler_to_load_path
    ...
    # puts "path? /home/vagrant/#{slug_vendor_base}/gems/"
    # puts Dir["/home/vagrant/#{slug_vendor_base}/gems/bundler-1.3.5/lib"].to_s
    # =>                                                  /usr/local/lib/ruby/gems/1.9.1/gems
    # res = run_with_err_output("cd vendor/bundle/ruby/1.9.1/ && ls -a  ")
    # print "res = #{res}"
    # puts "base= #{slug_vendor_base}"# = vendor/bundle/ruby/1.9.1     gems
    $: << File.expand_path(Dir["#{slug_vendor_base}/gems/bundler*/lib"].first)
    #("/usr/local/lib/ruby/gems/1.9.1/gems/bundler*/lib")
    # # here not exe?
     # print $:
